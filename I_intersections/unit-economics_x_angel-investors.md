---
type: intersection
id: unit-economics_x_angel-investors
aspect: unit-economics
audience: angel-investors
dimension: 3+
hooks:
- unit-economics—capital efficiency
- unit-economics—leveraged growth model
- unit-economics—margin expansion
- unit-economics—small account flywheel
- unit-economics—capacity trap
- unit-economics—advisory upsell
q_holes:
- unit-economics—founder quality and commitment
- unit-economics—problem-solution fit
- unit-economics—early traction and validation
- unit-economics—capital efficiency and scrappiness
- unit-economics—team capability and hustle
- unit-economics—market timing and opportunity
- unit-economics—path to next milestone
- unit-economics—expansion revenue
- unit-economics—CAC payback
bridges:
- Add `founder quality and commitment` as a synonym or linked facet on `unit-economics` (matches audience concern).
- Add `problem-solution fit` as a synonym or linked facet on `unit-economics` (matches audience concern).
- Add `early traction and validation` as a synonym or linked facet on `unit-economics` (matches audience concern).
- Add `capital efficiency and scrappiness` as a synonym or linked facet on `unit-economics` (matches audience concern).
- Add `team capability and hustle` as a synonym or linked facet on `unit-economics` (matches audience concern).
- Add `market timing and opportunity` as a synonym or linked facet on `unit-economics` (matches audience concern).
- Add `path to next milestone` as a synonym or linked facet on `unit-economics` (matches audience concern).
- Add `expansion revenue` as a synonym or linked facet on `unit-economics` (matches vocab_map term).
- Add `CAC payback` as a synonym or linked facet on `unit-economics` (matches vocab_map term).
message_invariants:
- Maintain consistent terminology aligned with audience vocabulary
- All claims must reference supporting evidence
risk_flags:
- Verify all q-holes are addressed before finalizing deliverable
---

# Intersection Summary

This intersection analyzes how **unit-economics** relates to the **angel-investors** audience. 
There are 6 connection point(s) (hooks) between the facet and audience concerns.
 There are 9 gap(s) (q-holes) that may need attention.

## Narrative Path (Traffic)

1. **Open with hooks**: Lead with the connection points that resonate with this audience's concerns.
2. **Address gaps**: Proactively address any q-holes by providing context or evidence.
3. **Close with action**: End with a clear call to action appropriate for this audience.

## Checklist

- [ ] Review all hooks for accuracy and relevance
- [ ] Address each q-hole with evidence or explanation
- [ ] Verify terminology matches audience vocab_map
- [ ] Check message invariants are maintained
- [ ] Validate against style guide